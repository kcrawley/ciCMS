...
class Auth extends CI_Model {
    ...
    public function validate_login($user, $pass) {
        // create query
        if ($stmt = $this->db->prepare("SELECT id, auth, username FROM tek_users WHERE username = :username AND password = :password")) {
    ...
    public function check_auth_level($challenge) {
        if ($this->session->userdata('logged_in')) {
            if (array_key_exists(strtolower($challenge), $this->auth_levels)) {
                return ($this->session->userdata('auth') >= $this->auth_levels[strtolower($challenge)]) ? TRUE : FALSE;
            }
        }
        return FALSE;
    }
    ...
    public function get_auth($username = NULL) {
        ...
        if ($stmt = $this->db->prepare("SELECT auth FROM tek_users WHERE username = :username")) {
    ...
    public function check_authorization() {
        if (!$this->session->userdata('logged_in')) {
            $this->Renderer->error('Unauthorized');
        }
    }
    ...
    public function get_all_users() {
        if ($results = $this->db->query("SELECT `id`, `first_name`, `last_name`, `email` FROM `tek_users`
            "))
    ...
    public function get_current_uid() {
        if ($stmt = $this->db->prepare("SELECT `id` FROM `tek_users` WHERE `username` = :username")) {
